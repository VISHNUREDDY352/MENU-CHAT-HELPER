// Curated food image map — fallback for items without image_url
const FOOD_IMAGES = {
  // Starters
...
function getImage(item) {
  ...
  return CATEGORY_FALLBACK[item.category] || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400&q=80'
}
          ...
          <section key={cat}>
            ...
            <div className="sr-cards">
              {group.map(item => {
                ...
                return (
                  <div key={item.id} className={`sr-card ${isHighlighted ? 'highlighted' : ''}`}>
                    ...
                    <img
                      ...
                      onError={e => {
                        ...
                        e.target.src = CATEGORY_FALLBACK[item.category] || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400&q=80'
                      }}
                    />
                    ...
                    <div className="sr-card-body">
                      ...
                      {item.description && (
                        <div className="sr-card-desc">{item.description}</div>
                      )}
                      <div className="sr-card-tags">
                        <span className={`sr-tag ${item.is_veg ? 'sr-tag-veg' : 'sr-tag-nonveg'}`}>
                          {item.is_veg ? '🌿 Veg' : '🍗 Non-veg'}
                        </span>
                        {item.is_spicy && (
                          <span className="sr-tag sr-tag-spicy">🌶 Spicy</span>
                        )}
                      </div>
                      <div className="sr-card-footer">
                        <span className="sr-price">₹{Number(item.price).toFixed(0)}</span>
                        <button className="sr-add-btn" onClick={() => onAdd(item)}>
                          + Add
                        </button>
                      </div>
                    </div>
